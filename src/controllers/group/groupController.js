...
import groupService from "../../services/group/group.service.js";
import {
...
export const create = async (req, res) => {
  try {
    const response = await groupService.create(req.body); // Creating a new group based on request body

    if (response.success) {
      res.status(200).json(
        generateResponseObject(true, CREATE_GROUP_SUCCESS, response.data) // Sending success response with newly created group object
      );
    } else
      res
        .status(500)
        .json(generateResponseObject(false, "Group with name already exists"));
  } catch (error) {
